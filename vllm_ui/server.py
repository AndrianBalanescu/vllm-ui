...
def get_models_info():
    try:
        ...
        with urllib.request.urlopen(req, timeout=3) as resp:
            ...
            return models if models else ["default"], max_model_len
    except Exception:
        return ["default"], None
...
class VLLMProxyHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/api/stats":
            ...
            if STATIC_DIR and (STATIC_DIR / self.path.lstrip("/")).exists():
                return super().do_GET()
            self.send_response(404)
            self.end_headers()

    def do_POST(self):
        if self.path == "/v1/chat/completions" or self.path == "/api/chat/completions":
